...
router.put('/:city_id', (req, res) => {
  if (!req.params.city_id || !req.body) {
  ...
  City.updateCity(req.params.city_id, req.body, function (err, result) {
    if (err) {
      ...
      console.info(`New city name: ${req.body.city_name}`);
...
router.put('/:city_id/categories/:cat_id', (req, res) => {
  if (!req.params.city_id || !req.params.cat_id || !req.body) {
...
// DELETE A ITEM FOE CATEGORY FOR CITY
...
router.put('/:city_id/categories/:cat_id/items/:item_id', (req, res) => {
  if (!req.params.city_id || !req.params.cat_id || !req.params.item_id || !req.body) {
    return res.status(400).send('Needs a name')
  }
